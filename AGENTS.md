...
- New agents = a new adapter under `src/annals/adapters/` plus a `[[source]] kind`. Do not special-case inside `cli.py`.
...
- Nothing operational goes in the archive worktree. Lock, log, and scheduler output live in `state_dir`; `git add -A` would otherwise commit them, so every run would produce a commit even with no new sessions.
